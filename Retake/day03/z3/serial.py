...
# cat serial.py | grep "if int" | sed -e "s/if int(serial/solver.add(s/g" | sed -e "s/\[//g" | sed -e "s/\]//g" | sed -e "s/int(serial/s/g" | sed -e "s/)//g" | sed -e "s/!=/==/g" | sed -e "s/:/)/g"

import sys
print ("Enter a valid serial for your purchase")
if len(sys.argv) < 2:
